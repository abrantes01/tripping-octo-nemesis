...
;(function($) {
    ...
    return this.each(function(index) {
      ...
      if (opts.killTitle) {
        $(this).find('[title]').andSelf().each(function() {
          if (!$(this).attr('bt-xTitle') && $(this).attr('title')) {
       ...
       */
      this.btOn = function () {
        if($(this).hasClass('bt-disabled')) return;
        ...
        var textTop, textLeft, textRight, textBottom, textTopSpace, textBottomSpace, textLeftSpace, textRightSpace, crossPoint, textCenter, spikePoint, strokeOffset={x:0,y:0};
        ...
        switch(position) {
          ...
          case 'top':
            ...
            points[points.length] = spikePoint = {y: textBottom + opts.spikeLength, x: ((textRight-textLeft) * .5) + xShift + opts.spikePaddingX, type: 'spike'};
          ...
          case 'left':
            ...
            if (textBottomSpace < 0 && !opts.noScroll) {
              // shift it up
            ...
            if (textTopSpace < 0 && !opts.noScroll) {
              // shift it down
            ...
            textBottom = textTop + $text.outerHeight();
            textCenter = {x: textLeft + ($text.btOuterWidth()*opts.centerPointX), y: textTop + ($text.outerHeight()*opts.centerPointY)};
            points[points.length] = spikePoint = {x: textRight + opts.spikeLength, y: ((textBottom-textTop) * .5) + yShift + opts.spikePaddingY, type: 'spike'};
          ...
          // =================== BOTTOM =======================
          case 'bottom':
            strokeOffset.y=opts.strokeWidth;
            // spike on top
            ...
            textBottom = textTop + $text.outerHeight();
            textCenter = {x: textLeft + ($text.btOuterWidth()*opts.centerPointX), y: textTop + ($text.outerHeight()*opts.centerPointY)};
            points[points.length] = spikePoint = {x: ((textRight-textLeft) * .5) + xShift + opts.spikePaddingX, y: shadowShiftY, type: 'spike'};
          ...
          // =================== RIGHT =======================
          case 'right':
            strokeOffset.x=opts.strokeWidth;
            // spike on left
            ...
            textBottom = textTop + $text.outerHeight();
            textCenter = {x: textLeft + ($text.btOuterWidth()*opts.centerPointX), y: textTop + ($text.outerHeight()*opts.centerPointY)};
            points[points.length] = spikePoint = {x: shadowShiftX, y: ((textBottom-textTop) * .5) + yShift + opts.spikePaddingY, type: 'spike'};
        ...
        for(var p in points){
          points[p].x+=strokeOffset.x;
          points[p].y+=strokeOffset.y;
  ...
  */
  ...
  /**
   *
   * A convenience function to enable bt
   * for each selected item
   */
  jQuery.fn.btEnable = function() {
    return this.removeClass('bt-disabled');
  };

  /**
   *
   * A convenience function to disable bt
   * for each selected item
   */
  jQuery.fn.btDisable = function() {
    return this.addClass('bt-disabled');
  };
  ...
  jQuery.bt.defaults = {
    ...
    spikePaddingX:    0,                     // move edge of the spike
    spikePaddingY:    0,
    ...
    noScroll:         false,                 // if true, position the box on the page without
